...
export class EventsService {
  async createEvent(userId: string, eventData: any) {
    try {
      await dbConnect();
      const prioriwiseCalendarExists =
  ...
  async getPrioriwiseEvents(userId: string) {
    try {
      ...
      const events = await getAllEventsForTwoWeeks(
        prioriwiseCalendarExists.auth,
        prioriwiseCalendarExists.prioriwiseCalendar.id,
      );
  ...
  async getPrioriwiseCalendarEvents(userId: string, calendarId: string) {
    try {
      ...
      console.error(
        "Error retrieving events for calendar {}:",
        calendarId,
        error,
      );
      throw error;
  ...
  async getUpcomingCalendarEventsFor(
    userId: string,
    ...
    try {
      ...
      for (const calendarId of calendarIds) {
        const events = await this.getAllEventsFor(
          calendarAuth.auth,
          calendarId,
        );
        if (events) {
  ...
  async getCalendarAuthForUserIfPrioriwiseCalendarExists(userId: string) {
    ...
    if (
      !prioriwiseCalendarExists ||
      !prioriwiseCalendarExists.prioriwiseCalendar ||
      !prioriwiseCalendarExists.prioriwiseCalendar.id
    ) {
  ...
  async getAllEventsFor(
    ...
    try {
      ...
      if (isNaN(max_hours)) {
        throw new Error(
          "REPRIORITIZE_EVENT_TIME_IN_HOURS must be a valid number",
        );
      }
      ...
      if (events) {
        console.log(
          `Found ${events.length} events for calendar ${calendarId} between ${startTime} and ${endTimeISO}`,
        );
      }
      return events;
    } catch (error) {
